...
template <typename Type> Matrix2D<Type>::Matrix2D(int _nrows, int _ncols, ModeOfMaterixInit _mode) {
...
template <typename Type> Matrix2D<Type>::Matrix2D(std::vector<std::vector<double>>& _matrixTable) {
	this->matrixTable = _matrixTable;
}

template <typename Type> int Matrix2D<Type>::getRowsNumber() {
	return matrixTable.size();
}

template <typename Type> std::vector<double>* Matrix2D<Type>::getRow(int _rowNumber) {
	return  &matrixTable.at(_rowNumber);
}

template <typename Type> void Matrix2D<Type>::normalize() {
...
template <typename Type> Matrix2D<Type> Matrix2D<Type>::getTransposedMatrix() {
...
template <typename Type> Matrix2D<Type> Matrix2D<Type>::operator+(Matrix2D<Type> _matrixToBeAdded) {
...
template <typename Type> Matrix2D<Type> Matrix2D<Type>::operator-(Matrix2D<Type> matrixToSubstracted) {
...
template <typename Type> template <typename Type2> Matrix2D<Type> Matrix2D<Type>::operator*(Type2 _value) {
	Matrix2D newMatrix(matrixTable.size(), matrixTable.at(0).size(), ModeOfMaterixInit::ZEROS);
	for (int i = 0; i < matrixTable.size(); ++i) {
		for (int j = 0; j < matrixTable.at(i).size(); ++j) {
			newMatrix.getRow(i)->at(j) = this->getRow(i)->at(j) * _value;
		}
	}
	return newMatrix;
}

template <typename Type> void Matrix2D<Type>::printMatrix() {
	for (int i = 0; i < matrixTable.size(); ++i) {
		for (int j = 0; j < matrixTable.at(i).size(); ++j) {
...
template <typename Type> std::vector<std::vector<Type>> Matrix2D<Type>::getTabel() {
	return matrixTable;
}